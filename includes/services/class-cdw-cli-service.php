...
class CDW_CLI_Service {
        ...
        switch ( $subcmd ) {
            ...
            case 'delete':
                ...
                if ( $theme->get_stylesheet() === wp_get_theme()->get_stylesheet() ) {
                    return array( 'output' => 'Cannot delete active theme.', 'success' => false );
                }
                require_once ABSPATH . 'wp-admin/includes/file.php';
                if ( ! $this->init_filesystem() ) {
            ...
            case 'install':
                ...
                if ( is_wp_error( $api ) ) {
                    return array( 'output' => 'Theme not found in repository: ' . $slug, 'success' => false );
                }
    ...
    private function handle_user_command( $subcmd, $args, $raw_args = array() ) {
        switch ( $subcmd ) {
            ...
            case 'create':
                if ( count( $args ) < 3 ) {
                    return array( 'output' => 'Usage: user create <username> <email> <role>', 'success' => false );
                }
                ...
                if ( username_exists( $username ) ) {
                    return array( 'output' => "User already exists: $username", 'success' => false );
                }
                if ( ! is_email( $email ) ) {
                    return array( 'output' => "Invalid email: $email", 'success' => false );
                }
                if ( ! get_role( $role ) && 'administrator' !== $role ) {
                    return array( 'output' => "Invalid role: $role", 'success' => false );
                }

                $user_id = wp_create_user( $username, wp_generate_password(), $email );
                if ( is_wp_error( $user_id ) ) {
                    return array( 'output' => 'User creation failed: ' . $user_id->get_error_message(), 'success' => false );
                }
            ...
            case 'delete':
                if ( empty( $args[0] ) ) {
                    return array( 'output' => 'Usage: user delete <username> --force', 'success' => false );
                }
            ...
            case 'role':
                if ( count( $args ) < 2 ) {
                    return array( 'output' => 'Usage: user role <username> <role>', 'success' => false );
                }
                $identifier = sanitize_text_field( $args[0] );
                ...
                return array( 'output' => "User role updated: $identifier -> $new_role", 'success' => true );
    ...
    private function handle_post_command( $subcmd, $args, $raw_args = array() ) {
        switch ( $subcmd ) {
            case 'list':
                $post_type = ! empty( $args[0] ) ? sanitize_text_field( $args[0] ) : 'post';
                $posts     = get_posts( array( 'post_type' => $post_type, 'posts_per_page' => 20 ) );
                if ( empty( $posts ) ) {
                    return array( 'output' => "No posts found for type: $post_type", 'success' => true );
                }
            ...
            case 'delete':
                if ( empty( $args[0] ) ) {
                    return array( 'output' => 'Usage: post delete <post-id>', 'success' => false );
                }
                ...
                if ( ! $result ) {
                    return array( 'output' => 'Delete failed', 'success' => false );
                }
    ...
    private function handle_option_command( $subcmd, $args ) {
        switch ( $subcmd ) {
            case 'get':
                if ( empty( $args[0] ) ) {
                    return array( 'output' => 'Usage: option get <name>', 'success' => false );
                }
            ...
            case 'set':
                if ( count( $args ) < 2 ) {
                    return array( 'output' => 'Usage: option set <name> <value>', 'success' => false );
                }
                $name  = sanitize_text_field( $args[0] );
            ...
            case 'delete':
                if ( empty( $args[0] ) ) {
                    return array( 'output' => 'Usage: option delete <name>', 'success' => false );
                }
                $name = sanitize_text_field( $args[0] );
                if ( $this->is_option_protected( $name ) ) {
    ...
    private function handle_transient_command( $subcmd, $args ) {
        switch ( $subcmd ) {
            ...
            case 'delete':
                if ( empty( $args[0] ) ) {
                    return array( 'output' => 'Usage: transient delete <name>', 'success' => false );
                }
                $key   = sanitize_text_field( $args[0] );
                $result = delete_transient( $key );
                return array( 'output' => $result ? "Transient deleted: $key" : "Transient not found: $key", 'success' => $result );
